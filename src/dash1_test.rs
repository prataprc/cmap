...
macro_rules! test_code {
    ($seed:expr, $keytype:ty) => {{
        ...
        println!(
            "test_dash1_map_{} seed:{} key_max:{} ops:{} threads:{} modul:{}",
            stringify!($keytype),
            $seed,
            key_max,
            n_ops,
        ...
        println!("test_dash1_map_{} len {}", stringify!($keytype), map.len());
        assert_eq!(map.len(), dmap.len());
        println!(
            "test_dash1_map_{} Validate .... {:?}",
            stringify!($keytype),
            map.validate()
        );
...
where
    K: Key,
{
    // we will adjust the key such that same key will not be used by other threads.
    fn adjust_op(self, id: K, modul: K) -> Self {
        match self {
            Op::Get(key) => {
